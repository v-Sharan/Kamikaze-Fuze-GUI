  ...
  (event) => (dispatch, getState) => {
    ...
    switch (reason) {
      case 'connection error':
        if (!willReconnect) {
          dispatch(showError('Error while connecting to XAG Server'));
        }
      ...
      case 'connection timeout':
        if (!willReconnect) {
          dispatch(showError('Timeout while connecting to XAG Server'));
        }

        break;

      case 'io client disconnect':
        dispatch(showNotification('Disconnected from XAG Server'));
        break;
      ...
      case 'transport close':
        dispatch(showError('XAG Server closed connection unexpectedly'));
        break;

      case 'ping timeout':
        dispatch(showError('Connection to XAG Server lost'));
        break;
  ...
  (dispatch) => ({
    ...
    onLocalServerError(message, wasRunning) {
      const baseMessage = wasRunning
        ? 'XAG Server died unexpectedly'
        : 'Failed to launch local XAG Server';
      dispatch(setCurrentServerConnectionState(ConnectionState.DISCONNECTED));
